...
def build_products_agent(cfg: OpenAIConfig,  model: str = MODEL_MAIN) -> Agent:

    product_prompt = insert_txt_in_block(PRODUCT_PROMPT_PATH, cfg.catalogs_file, '<<CATALOGS_BLOCK>>')
    product_prompt = insert_style_in_prompt(product_prompt)
    product_prompt = insert_company_info_in_prompt(product_prompt, cfg)
    return Agent(
        name="Products Agent",
        model=model,
        handoff_description=(
